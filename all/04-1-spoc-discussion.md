...
1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？
  ...
  > 让程序中不会同时运行的代码使用同一块内存，开发者需要确定内存共享关系

2. 什么是交换技术？覆盖与交换有什么不同？

  > 将当前不使用的内存区域的内容转移到外存中，以在对应区域存放其他内容

3. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？

  > 覆盖：嵌入式系统，交换：物理内存临时不够用
...
1. 什么是虚拟存储？它与覆盖和交换的区别是什么？它有什么好处和挑战？
  ...
  > 按需分配，只给当前使用的内存分配物理页，并支持部分的换出，物理上不连续。能够节省内存，但是实现比较繁琐。

2. 虚拟存储需要什么样的支持技术？

  > 处理器支持硬件的地址转换（MMU），并且 OS 需要实现合适的置换算法。
...
### 8.5 虚拟页式存储
 ...
 1. 什么是虚拟页式存储？缺页中断处理的功能是什么？
  ...
  > 动态加载页面，并支持换出。用于加载当前不在内存中的页面。

 2. 为了支持虚拟页式存储的实现，页表项有什么修改？

  > 需要有 Present、Dirty、Access、Protection 位。


 3. 页式存储和虚拟页式存储的区别是什么？

  > 页式存储只是将内存分成页面，所有进程共享地址空间；而虚拟页式存储对于每个进程呈现唯一的地址空间。

### 8.6 缺页异常

1. 缺页异常的处理流程？

  > 检查权限，在外存中定位到页面，换入，修改页表，并返回。

2. 虚拟页式存储管理中有效存储访问时间是如何计算的？

  > 访存时间 * (1 - 缺页率) + 异常处理时间 * 缺页率
